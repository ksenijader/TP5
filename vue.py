...
class Application(Tk):
    ...
    def creer_widgets(self):
        ...
        self.bouton_add_animal = Button(self, text="Add",
    ...
    def edit_animal(self):
        ...
        edit_window.title("Edit animal information")
        ...
        for k in entries_dict:
            for att in self.attributes:
                ...
                if k == att:
                    self.entries[att] = Entry(edit_window)
                    self.entries_label[att] = Label(edit_window, text=att)
                    # Fill the entry field with information
                    # stocked in entries_dict
                    self.entries[att].insert(1, entries_dict[k])
                    self.entries_label[att].pack()
                    self.entries[att].pack()
        # Create button that will apply changes and
        # close the window once done
        edit_window.apply_button = Button(edit_window, text="Apply",
                                          command=lambda: [self.add_animal(),
                                                           edit_window.destroy()])
        edit_window.apply_button.pack()
